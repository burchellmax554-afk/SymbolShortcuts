...
@********************************************************************************************************
    ...
    .global  PendSV_Handler                                     @ CMSIS compliant name for Cortex-M
...
OSStartHighRdy:
    ...
    LDR     R0, [R2, #0xC]                                      @ R0 = OSTCBHighRdy->StkBasePtr
...
.thumb_func
PendSV_Handler:                                                 @ CMSIS compliant name for Cortex-M
    CPSID     I
    ...
    LDR       R12, [R2]
    CMP       R12, #0
    BEQ       OS_CPU_BASEPRI_NoSave
    MSR       BASEPRI, R12
    DSB
    ISB
    CPSIE     I

OS_CPU_BASEPRI_NoSave:
    ...
    LDR       R0, [R2, #0xC]                                    @ R2 = OSTCBHighRdy->StkBasePtr
    RSB       R0, R0, #0                                        @ Align R0 downwards (in stack) to 8-bytes
    BIC       R0, #0x7                                          @ "                                      "
    RSB       R0, R0, #0                                        @ "                                      "
    MSR       PSPLIM, R0                                        @ Set the stack limit register

    MOVW      R2, #:lower16:OS_KA_BASEPRI_Boundary              @ Get BASEPRI priority level
    MOVT      R2, #:upper16:OS_KA_BASEPRI_Boundary
    LDR       R12, [R2]
    CMP       R12, #0
    BEQ       OS_CPU_BASEPRI_NoRestore
