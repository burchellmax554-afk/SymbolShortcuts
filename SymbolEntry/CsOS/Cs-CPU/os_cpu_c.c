...
#if (!((OS_CFG_DBG_EN > 0u) || (OS_CFG_STAT_TASK_STK_CHK_EN > 0u) || (OS_CFG_TASK_STK_REDZONE_EN > 0u) || (OS_CFG_TASK_STK_EXT_INFO > 0)))
#error "StkBasePtr must be enabled in TCBs to use the ARMv8-M port"
#endif
...
void  OSInitHook (void)
{
    ...
    CPU_SetMSPLIM((CPU_ADDR)OSCfg_ISRStkBasePtr);
...
*                                         STATISTIC TASK HOOK
*
* Description: This function is called every second by Cs/OS3's statistics task.  This allows your
...
*
*              (5) Exception Return Behavior(EXC_RETURN)
*
*                    (a) If running in Secure Mode
*
...
*
*                    (b) If running in Non-Secure Mode or the Security Extension is not implemented
*
*                          0xFFFFFFBC      Return to Thread mode, exception return uses non-floating point state
*                                          from the PSP and execution uses PSP after return.
...
#if (OS_CPU_ARM_CMSE_SECURE > 0u)
    *(--p_stk) = (CPU_STK)0xFFFFFFFDuL;                         /* R14: EXC_RETURN; See Note 5a                         */
#else
    *(--p_stk) = (CPU_STK)0xFFFFFFBCuL;                         /* R14: EXC_RETURN; See Note 5b                         */
#endif
...
*                                          SYS TICK HANDLER
*
* Description: Handle the system tick (SysTick) interrupt, which is used to generate the Cs/OS3 tick
*              interrupt.
*
* Arguments  : None.
*
...
void  OS_CPU_SysTickHandler  (void)
{
    ...
    CPU_CRITICAL_ENTER();
    OSIntEnter();                                               /* Tell Cs/OS3 that we are starting an ISR              */
    CPU_CRITICAL_EXIT();

    OSTimeTick();                                               /* Call Cs/OS3's OSTimeTick()                           */

    OSIntExit();                                                /* Tell Cs/OS3 that we are leaving the ISR              */
}


/*
*********************************************************************************************************
*                                   CMSIS COMPLIANT SYS TICK HANDLER
*
* Description: Wrapper function to handle the system tick (SysTick) interrupt, which is used to generate
*              the Cs/OS3 tick interrupt.
*
* Arguments  : None.
*
* Note(s)    : 1) This function MUST be placed on entry 15 of the Cortex-M vector table.
*              2) This function follows the CMSIS compliant name for Cortex-M.
*********************************************************************************************************
*/
...
#else
    basepri             = OS_CPU_CFG_SYSTICK_PRIO;
#endif
