...
#define  CPU_INT_DIS()         do { cpu_sr = CPU_SR_IPLSave(CPU_CFG_KA_IPL_BOUNDARY << (8u - CPU_CFG_NVIC_PRIO_BITS));} while (0)
#define  CPU_INT_EN()          do { CPU_SR_IPLRestore(cpu_sr); } while (0) /* Restore CPU BASEPRI priority level.       */
#else
...
#if (CPU_CFG_KA_IPL_BOUNDARY > 0u)
CPU_SR      CPU_SR_IPLSave   (CPU_SR      new_basepri);
...
CPU_ADDR    CPU_SetMSPLIM    (CPU_ADDR    lim);
CPU_ADDR    CPU_SetPSPLIM    (CPU_ADDR    lim);
...
#define  CPU_REG_SCB_SHCSR_SECFAULTENA            0x00080000
