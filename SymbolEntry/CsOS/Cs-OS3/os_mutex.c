...
#if (OS_CREATE_EXT > 0u)
    p_mutex->CreateOpt = OS_OPT_CREATE_PRIO;
#endif
...
/*
************************************************************************************************************************
*                                              CREATE A MUTEX (Extended)
*
* Description: This function creates a mutex and provides an option field to configure how it behaves.
*
* Arguments  : p_mutex       is a pointer to the mutex to initialize.  Your application is responsible for allocating
*                            storage for the mutex.
*
*              p_name        is a pointer to the name you would like to give the mutex.
*
*              opt           specifies the options used when creating the mutex
*
                   ...
                   OS_ERR    *p_err)
{
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Don't allow blocking-pends from Timer Callbacks      */
        if ((opt & OS_OPT_PEND_NON_BLOCKING) != OS_OPT_PEND_NON_BLOCKING) {
            OS_TRACE_MUTEX_PEND_FAILED(p_mutex);
            OS_TRACE_MUTEX_PEND_EXIT(OS_ERR_PEND_TMR);
...
*
* Description: These functions are called by Cs/OS3 to add or remove a mutex to/from the debug list.
*
