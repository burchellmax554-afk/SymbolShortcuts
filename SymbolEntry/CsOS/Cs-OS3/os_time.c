...
*                                                       must reach before the task will be resumed.
*                            OS_OPT_TIME_PERIODIC_RESET reset the task's periodic base for the next periodic dly
*
*
...
*                            OS_ERR_TIME_DLY_TMR      If you called this function from a Timer Callback
                 ...
                 OS_ERR   *p_err)
{
    ...
    *p_err = OS_ERR_TICK_DISABLED;
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Don't allow delays from Timer Callbacks              */
       *p_err = OS_ERR_TIME_DLY_TMR;
        return;
    }
#endif
...
#if (OS_CFG_TICK_EN > 0u)
    CPU_CRITICAL_ENTER();
    if ((opt & OS_OPT_TIME_PERIODIC_RESET) != 0u) {
        OSTCBCurPtr->TickCtrSet = OS_FALSE;
    }
...
*                            OS_ERR_TIME_DLY_TMR                If called from a Tmr Callback
...
*                                                 PROCESS SYSTEM TICK
*
* Description: This function is used to signal to Cs/OS3 the occurrence of a 'system tick' (also known as a
