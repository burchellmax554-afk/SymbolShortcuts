...
*                                                    INITIALIZATION
*
* Description: This function is used to initialize the internals of Cs/OS3 and MUST be called prior to
*              creating any Cs/OS3 object and, prior to calling OSStart().
*
...
*                                                      ENTER ISR
*
* Description: This function is used to notify Cs/OS3 that you are about to service an interrupt service routine
*              (ISR).  This allows Cs/OS3 to keep track of interrupt nesting and thus only perform rescheduling at
*              the last nested ISR.
*
* Arguments  : none
*
...
*                                                       EXIT ISR
*
* Description: This function is used to notify Cs/OS3 that you have completed servicing an ISR.  When the last nested
*              ISR has completed, Cs/OS3 will call the scheduler to determine whether a new, high-priority task, is
*              ready to run.
*
* Arguments  : none
*
...
*                                                      SCHEDULER
*
* Description: This function is called by other Cs/OS3 services to determine whether a new, high priority task has
...
*                                                 START MULTITASKING
*
* Description: This function is used to start the multitasking process which lets Cs/OS3 manage the task that you
...
void  OSStart (OS_ERR  *p_err)
{
    ...
    kernel_task_cnt = 1u;                                       /* Calculate the number of kernel tasks                 */
...
*                                                    GET VERSION
*
* Description: This function is used to return the version number of Cs/OS3.  The returned value corresponds to
*              Cs/OS3's version number multiplied by 10000.  In other words, version 3.09.01 would be returned as 30901.
*
...
*                         OS_ERR_NONE
*
* Returns    : The version number of Cs/OS3 multiplied by 10000.
*
* Note(s)    : none
************************************************************************************************************************
*/
...
*                                                      IDLE TASK
*
* Description: This task is internal to Cs/OS3 and executes whenever no other higher priority tasks executes because
...
*                 Cs/OS3 would thus never recognize interrupts.
*
...
#if (OS_CREATE_EXT > 0u)
        if (p_obj->CreateOpt == OS_OPT_CREATE_PRIO) {           /* Order of insertion depends on the Create option      */
            OS_PendListInsertPrio(p_pend_list,
                                  p_tcb);
        } else {
            OS_PendListInsertFIFO(p_pend_list,
                                  p_tcb);
        }
#else
...
* Returns    : none
*
* Note(s)    : 1) These functions are INTERNAL to Cs/OS3 and your application must not call it.
************************************************************************************************************************
*/
...
#if (OS_CREATE_EXT > 0u)
    if (p_obj->CreateOpt == OS_OPT_CREATE_FIFO) {               /* FIFO ordered pend lists are not affected.            */
        return;
    }
#endif
...
/*
************************************************************************************************************************
*                                   INSERT A TASK IN A PEND LIST IN FIFO ORDER
*
* Description: This function is called to place an OS_TCB entry in a linked list in FIFO order.
*
*              CASE 0: Insert in an empty list.
*
*                     OS_PEND_LIST
...
*
*
*              CASE 1: Insert in a non-empty list.
*
*                     OS_PEND_LIST
*                     +--------------+         OS_TCB
*                     | TailPtr      |--+---> +--------------+
...
#if (OS_CFG_DBG_EN > 0u)
        p_pend_list->NbrEntries++;                              /* CASE 1: One or more OS_TCBs in the list              */
#endif
        p_tcb->PendNextPtr              = (OS_TCB *)0;          /* Insert at the tail.                                  */
        p_tcb->PendPrevPtr              =  p_pend_list->TailPtr;
        p_tcb->PendPrevPtr->PendNextPtr =  p_tcb;
               ...
               CPU_TS        ts)
{
    ...
    switch (p_tcb->TaskState) {
        ...
        case OS_TASK_STATE_PEND_TIMEOUT:
#if (OS_MSG_EN > 0u) || (OS_CFG_DATA_EN > 0u)
             if (p_obj == (OS_PEND_OBJ *)0) {                   /* Pending on TaskQ?                                    */
                 p_tcb->MsgPtr  = p_void;                       /* Yes, Deposit message in OS_TCB of task waiting       */
                 p_tcb->MsgSize = msg_size;                     /*      ... assuming posting a message                  */
             } else if (p_obj->Type == OS_OBJ_TYPE_Q) {         /* No,  Is it a message queue?                          */
                 p_tcb->MsgPtr  = p_void;                       /*      Yes, Deposit message in OS_TCB of task waiting  */
                 p_tcb->MsgSize = msg_size;                     /*            ... assuming posting a message            */
#if (OS_CFG_DATA_EN > 0u)
             } else if (p_obj->Type == OS_OBJ_TYPE_DATA) {      /* No,  Is it a data queue?                             */
                 p_source = (CPU_INT08U *)p_void;
                 p_dest   = (CPU_INT08U *)p_tcb->MsgPtr;
                 for (i = 0; i < msg_size; i++) {               /* Copy data to destination buffer                      */
                     *p_dest = *p_source;
                      p_dest++;
                      p_source++;
                 }
...
* Returns    : none
*
* Note(s)    : 1) The are internal functions to Cs/OS3 and MUST not be called by your application code.
*
