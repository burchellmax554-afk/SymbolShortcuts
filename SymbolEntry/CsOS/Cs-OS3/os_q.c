...
#if (OS_CREATE_EXT > 0u)
    p_q->CreateOpt = OS_OPT_CREATE_PRIO;
#endif
...
/*
************************************************************************************************************************
*                                         CREATE A MESSAGE QUEUE (Extended)
*
* Description: This function creates a Message Queue and provides an option field to configure how it behaves.
*
* Arguments  : p_q         is a pointer to the message queue
*
*              p_name      is a pointer to an ASCII string that will be used to name the message queue
...
*                          possible to have a size higher than the maximum number of OS_MSGs available.
*
*              opt         specifies the options used when creating the message queue
*
...
*
* Note(s)     : 1) You should use this function with great care because, when to flush the queue, you LOSE the
                ...
                OS_ERR       *p_err)
{
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Don't allow blocking-pends from Timer Callbacks      */
        if ((opt & OS_OPT_PEND_NON_BLOCKING) != OS_OPT_PEND_NON_BLOCKING) {
            OS_TRACE_Q_PEND_FAILED(p_q);
            OS_TRACE_Q_PEND_EXIT(OS_ERR_PEND_TMR);
...
*
* Description: These functions are called by Cs/OS3 to add or remove a message queue to/from a message queue debug
*              list.
*
