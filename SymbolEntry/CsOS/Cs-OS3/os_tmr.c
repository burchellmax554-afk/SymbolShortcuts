...
*
*                                  void  MyCallback (void *p_tmr, void *p_arg);
*
...
* Returns    : none
*
* Note(s)    : 1) This function is INTERNAL to Cs/OS3 and your application MUST NOT call it.
************************************************************************************************************************
*/
...
/*
************************************************************************************************************************
*                                              ADD TIMER TO DEBUG LIST
*
* Description: This function is called by Cs/OS3 to add a timer to the timer debug list.
*
...
/*
************************************************************************************************************************
*                                            REMOVE TIMER FROM DEBUG LIST
*
* Description: This function is called by Cs/OS3 to remove a timer from the timer debug list.
*
* Arguments  : p_tmr     is a pointer to the timer to add/remove
*
* Returns    : none
...
void  OS_TmrInit (OS_ERR  *p_err)
{
    ...
    OS_TmrTaskLock();                                           /* Lock is owned by TmrTask once the OS starts.         */
}
...
void  OS_TmrTask (void  *p_arg)
{
    ...
    (void)p_arg;                                                /* Not using 'p_arg', prevent compiler warning.         */
                                                                ...
                                                                /* Tmr Mutex is held upon entry.                        */
    for (;;) {
        CPU_CRITICAL_ENTER();                                   /* Ensure TmrTask can Sched() before HPT signals us.    */
        OS_TmrTaskUnlock();                                     /* Allow access to the timer list.                      */
        OS_TmrTaskSched();                                      /* Schedule the next timeout.                           */
        CPU_CRITICAL_EXIT();

        OSSched();

        OS_TmrTaskLock();                                       /* Lock access to the tmr list.                         */

        if (OSTmrListPtr == (OS_TMR *)0) {                      /* Only proceed if any timers need to be processed.     */
            continue;
        }
...
*                                                    ACQUIRE LOCK
*
* Description: Used by the OSTmr functions to request exclusive access to the timer list.
*
* Arguments  : none
*
* Returns    : none
...
*                                                    RELEASE LOCK
*
* Description: Used by the OSTmr functions to relinquish exclusive access to the timer list.
*
* Arguments  : none
*
* Returns    : none
...
/*
************************************************************************************************************************
*                                            ACQUIRE TMR MUTEX (TMR TASK)
*
* Description: Helper function which grants ownership of the Tmr mutex to the Tmr Task. If the mutex is already owned
*              the TmrTask is instead added to the pend list.
*
* Arguments  : none
*
* Returns    : none
...
void  OS_TmrTaskLock (void)
{
    ...
    if (OSTmrMutex.OwnerTCBPtr == (OS_TCB *)0) {                /* Can we grab the mutex?                               */
        OSTmrMutex.OwnerTCBPtr     = &OSTmrTaskTCB;             /* Yes, no-one else pending.                            */
        OSTmrMutex.OwnerNestingCtr =  1u;
        OS_MutexGrpAdd(&OSTmrTaskTCB, &OSTmrMutex);
        CPU_CRITICAL_EXIT();
    } else {
...
/*
************************************************************************************************************************
*                                            RELEASE TMR MUTEX (TMR TASK)
*
* Description: Helper function which releases ownership of the Tmr mutex from the Tmr Task.
*
* Arguments  : none
*
* Returns    : none
...
void  OS_TmrTaskUnlock (void)
{
    ...
    OS_MutexGrpRemove(&OSTmrTaskTCB, &OSTmrMutex);              /* Remove mutex from TmrTask's group                    */
...
/*
************************************************************************************************************************
*                                           SCHEDULE THE TIMER TASK WAKEUP
*
* Description: Schedules the timer task's next wakeup based on the active timers. The amount of time TmrTask will sleep
*              may be indefinite or finite depending on the state of the timer list.
*
*                  (1) If the timer list is empty, TmrTask has nothing to do and will suspend until a timer is
*                      available.
*
*                  (2) If the timer list is non-empty, TmrTask will delay until the next timer expires.
*
*               TmrTask should check our return and decide whether or not to call OSSched().
*
* Arguments  : none.
*
* Returns    : OS_TRUE          if TmrTask needs to sleep
*
*              OS_FALSE         otherwise
*
* Note(s)    : 1) This function is INTERNAL to Cs/OS3 and your application MUST NOT call it.
...
void  OS_TmrTaskSched (void)
{
    ...
    if (OSTmrListPtr == (OS_TMR *)0) {                          /* TmrTask needs to pend indefinitely                   */
        OSTmrTaskTCB.TaskState  = OS_TASK_STATE_SUSPENDED;
        OSTmrTaskTCB.SuspendCtr = 1u;
        OS_RdyListRemove(&OSTmrTaskTCB);
    } else {
#if (OS_CFG_DYN_TICK_EN > 0u)
        elapsed = OS_DynTickGet();
        time    = OSTickCtr + elapsed;
#else
        elapsed = 0u;
        time    = OSTickCtr;
#endif

        (void)OS_TickListInsert(&OSTmrTaskTCB,
                                 elapsed,
                                 time,
                                 OSTmrListPtr->Remain);

        OSTmrTaskTCB.TaskState = OS_TASK_STATE_DLY;
        OS_RdyListRemove(&OSTmrTaskTCB);
    }
}
...
*
* Description: Force TmrTask into the Ready state. Used when adding/removing a timer to the list invalidates
*              the timer task's expected wake-up time.
*
* Arguments  : none.
*
* Returns    : none
...
* Note(s)    : 1) This function is INTERNAL to Cs/OS3 and your application MUST NOT call it.
*
*              2) Set of states entered by TmrTask during it's lifecycle
*                  RDY
*                  DLY
*                  SUSPENDED
*                  PEND
...
void  OS_TmrTaskSignal (void)
{
    ...
    CPU_CRITICAL_ENTER();
    switch (OSTmrTaskTCB.TaskState) {
        case OS_TASK_STATE_DLY:                                 /* TmrTask delayed until tmr expiration. Wake it early. */
             OS_TickListRemove(&OSTmrTaskTCB);
             OS_RdyListInsert(&OSTmrTaskTCB);
             OSTmrTaskTCB.TaskState = OS_TASK_STATE_RDY;
             break;

        case OS_TASK_STATE_SUSPENDED:                           /* TmrTask suspended indefinitely; Force it to resume.  */
             OS_RdyListInsert(&OSTmrTaskTCB);
             OSTmrTaskTCB.TaskState = OS_TASK_STATE_RDY;
             break;

        case OS_TASK_STATE_RDY:                                 /* TmrTask will run when scheduled by kernel.           */
        case OS_TASK_STATE_PEND:                                /* TmrTask waiting to process timers.                   */
        default:
