...
*              The function is internal to Cs/OS3.
*
...
*                                                      INSERT
*
* Description: This task is internal to Cs/OS3 and allows the insertion of a task in a tick list.
*
...
#if (OS_CFG_DYN_TICK_EN > 0u)                                   /* tick_base represents the current system time.        */
        tick_base = OSTickCtr + elapsed;
#else
        tick_base = OSTickCtr;
#endif

        if (p_tcb->TickCtrSet == OS_FALSE) {                    /* If this is the task's first Periodic delay:          */
            p_tcb->TickCtrPrev  = tick_base + time;             /*  Init TickCtrPrev.                                   */
            p_tcb->TickCtrSet   = OS_TRUE;                      /*  Mark the Counter as initialized.                    */
        } else if ((tick_base - p_tcb->TickCtrPrev) <= time) {  /* If we're still within the period boundary:           */
            tick_base           = p_tcb->TickCtrPrev;           /*  Next Wakeup: TickCtrPrev + time.                    */
            p_tcb->TickCtrPrev += time;                         /*  A zero delay will return immediately.               */
        } else {                                                /* If we've missed the wakeup:                          */
            p_tcb->TickCtrPrev += time;                         /*  Advance to the next Period.                         */
           *p_err               = OS_ERR_TIME_PERIOD_MISSED;    /*  Notify the caller that we missed the period.        */
            return;
        }
