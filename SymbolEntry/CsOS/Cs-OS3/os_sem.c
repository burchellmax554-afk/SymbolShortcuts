...
#if (OS_CREATE_EXT > 0u)
    p_sem->CreateOpt = OS_OPT_CREATE_PRIO;
#endif
...
/*
************************************************************************************************************************
*                                           CREATE A SEMAPHORE (Extended)
*
* Description: This function creates a semaphore and provides an option field to configure how it behaves.
*
* Arguments  : p_sem         is a pointer to the semaphore to initialize.  Your application is responsible for
*                            allocating storage for the semaphore.
*
...
*                            If used to signal the occurrence of event(s) then you should initialize to 0.
*
*              opt           specifies the options used when creating the semaphore
*
                       ...
                       OS_ERR   *p_err)
{
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Don't allow blocking-pends from Timer Callbacks      */
        if ((opt & OS_OPT_PEND_NON_BLOCKING) != OS_OPT_PEND_NON_BLOCKING) {
            OS_TRACE_SEM_PEND_FAILED(p_sem);
            OS_TRACE_SEM_PEND_EXIT(OS_ERR_PEND_TMR);
...
*
* Description: These functions are called by Cs/OS3 to add or remove a semaphore to/from the debug list.
*
