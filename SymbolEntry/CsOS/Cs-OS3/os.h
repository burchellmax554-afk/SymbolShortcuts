...
/*
************************************************************************************************************************
*                                                Cs/OS3 VERSION NUMBER
************************************************************************************************************************
*/

#define  OS_VERSION  30902u                       /* Version of Cs/OS3 (Vx.yy.zz mult. by 10000)                      */
...
#define  OS_CREATE_EXT             (((OS_CFG_DATA_CREATE_EXT  > 0u) || \
                                     (OS_CFG_MUTEX_CREATE_EXT > 0u) || \
                                     (OS_CFG_Q_CREATE_EXT     > 0u) || \
                                     (OS_CFG_SEM_CREATE_EXT   > 0u)) ? 1u : 0u)
...
#define  OS_TASK_PEND_ON_DATA                 (OS_STATE)(  8u)  /* Pending on data queue                              */
...
#define  OS_OBJ_TYPE_DATA                    (OS_OBJ_TYPE)CPU_TYPE_CREATE('D', 'A', 'T', 'A')
...
#define  OS_OPT_CREATE_PRIO                  (OS_OPT)(0x0000u)
#define  OS_OPT_CREATE_FIFO                  (OS_OPT)(0x0001u)
...
#define  OS_OPT_PEND_PEEK                    (OS_OPT)(0x4000u)  /* Look at next available entry without removing it   */
...
#define  OS_OPT_POST_FIFO_LIFO_MSK           (OS_OPT)(0x00F0u)  /* Mask to remove unwanted bits from FIFO/LIFO select */
#define  OS_OPT_POST_FIFO                    (OS_OPT)(0x0000u)  /* Post First-In-First-Out                            */
#define  OS_OPT_POST_LIFO                    (OS_OPT)(0x0010u)  /* Post Last-In-First-Out                             */
#define  OS_OPT_POST_FIFO_OVER               (OS_OPT)(0x0020u)  /* Overwrite FIFO                                     */
#define  OS_OPT_POST_LIFO_OVER               (OS_OPT)(0x0030u)  /* Overwrite LIFO                                     */
...
#define  OS_OPT_TIME_PERIODIC_RESET         ((OS_OPT)0x20u)
...
typedef  enum  os_err {
    ...
    OS_ERR_PEND_TMR                  = 25009u,
    OS_ERR_PEND_EMPTY                = 25010u,
    ...
    OS_ERR_TIME_DLY_TMR              = 29311u,
    OS_ERR_TIME_PERIOD_MISSED        = 29312u,
...
typedef  struct  os_data             OS_DATA;
...
struct  os_data {
#if (OS_OBJ_TYPE_REQ > 0u)
    OS_OBJ_TYPE          Type;                              /* Should be set to OS_OBJ_TYPE_DATA                      */
#endif
#if (OS_CFG_DBG_EN > 0u)
    CPU_CHAR            *NamePtr;                           /* Pointer to Data Queue Name (NUL terminated ASCII)      */
#endif
    OS_PEND_LIST         PendList;                          /* List of tasks waiting for the data queue               */
#if (OS_CFG_DBG_EN > 0u)
    OS_DATA             *DbgPrevPtr;
    OS_DATA             *DbgNextPtr;
    CPU_CHAR            *DbgNamePtr;
    ...
    OS_OPT               CreateOpt;
#endif
    CPU_VOID            *StorageBaseAddr;
    OS_MSG_QTY           InIx;
    OS_MSG_QTY           OutIx;
    OS_MSG_SIZE          ItemSize;
    OS_MSG_QTY           MaxEntries;
    OS_MSG_QTY           Entries;
#if (defined(OS_CFG_TRACE_EN) && (OS_CFG_TRACE_EN > 0u))
    CPU_INT16U           DataID;                            /* Unique ID for third-party debuggers and tracers.       */
#endif
};
...
struct os_tcb {
...
#if ((OS_CFG_DBG_EN > 0u) || (OS_CFG_STAT_TASK_STK_CHK_EN > 0u) || (OS_CFG_TASK_STK_REDZONE_EN > 0u) || (OS_CFG_TASK_STK_EXT_INFO > 0u))
    ...
    CPU_BOOLEAN          TickCtrSet;                        /* Used by OSTimeDlyXX() in PERIODIC mode                 */
#endif
...
#if (OS_MSG_EN > 0u) || (OS_CFG_DATA_EN > 0u)
...
#if (OS_CFG_DBG_EN  > 0u)
OS_EXT            OS_DATA                  *OSDataDbgListPtr;
OS_EXT            OS_OBJ_QTY                OSDataQty;
#endif
#endif
...
OS_EXT            OS_TICK                   OSTickCtrStep;              /* Number of ticks to the next tick interrupt */
#endif
...
#if (OS_CFG_MUTEX_CREATE_EXT > 0u)
void          OSMutexCreateExt          (OS_MUTEX              *p_mutex,
                                         CPU_CHAR              *p_name,
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);
#endif
...
#if (OS_CFG_Q_CREATE_EXT > 0u)
void          OSQCreateExt              (OS_Q                  *p_q,
                                         CPU_CHAR              *p_name,
                                         OS_MSG_QTY             max_qty,
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);
...
#if (OS_CFG_SEM_CREATE_EXT > 0u)
void          OSSemCreateExt            (OS_SEM                *p_sem,
                                         CPU_CHAR              *p_name,
                                         OS_SEM_CTR             cnt,
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);
...
void          OS_TmrLock                (void);

void          OS_TmrUnlock              (void);

void          OS_TmrTaskLock            (void);

void          OS_TmrTaskUnlock          (void);

void          OS_TmrTaskSched           (void);

void          OS_TmrTaskSignal          (void);
#endif
...
void       OS_TLS_GlobalAcquire         (void);

void       OS_TLS_GlobalRelease         (void);
#endif
...
************************************************************************************************************************
************************************************************************************************************************
*                        C s / O S 3   I N T E R N A L   F U N C T I O N   P R O T O T Y P E S
************************************************************************************************************************
************************************************************************************************************************
*/
...
#if (OS_CREATE_EXT > 0u)
void          OS_PendListInsertFIFO     (OS_PEND_LIST          *p_pend_list,
                                         OS_TCB                *p_tcb);
#endif
...
#ifndef OS_CFG_CALLED_FROM_TMR_CHK_EN
#error  "OS_CFG.H, Missing OS_CFG_CALLED_FROM_TMR_CHK_EN: Enable (1) or Disable (0) checking whether in an ISR in kernel services"
#endif
...
#ifndef OS_CFG_DATA_EN
#error  "OS_CFG.H, Missing OS_CFG_DATA_EN: Enable (1) or Disable (0) code generation for Data Queues"
#else
    #ifndef OS_CFG_DATA_DEL_EN
    #error  "OS_CFG.H, Missing OS_CFG_DATA_DEL_EN: Include code for OSDataDel()"
    #endif

    #ifndef OS_CFG_DATA_FLUSH_EN
    #error  "OS_CFG.H, Missing OS_CFG_DATA_FLUSH_EN: Include code for OSDataFlush()"
    #endif

    #ifndef OS_CFG_DATA_PEND_ABORT_EN
    #error  "OS_CFG.H, Missing OS_CFG_DATA_PEND_ABORT_EN: Include code for aborting pends from another task"
    #endif

    #ifndef OS_CFG_DATA_CREATE_EXT
    #error  "OS_CFG.H, Missing OS_CFG_DATA_CREATE_EXT: Include code for OSDataCreateExt()"
    #endif
#endif
...
#error  "OS_CFG.H, Missing OS_CFG_MUTEX_EN: Enable (1) or Disable (0) code generation for MUTEX"
    ...
    #ifndef OS_CFG_MUTEX_CREATE_EXT
    #error  "OS_CFG.H, Missing OS_CFG_MUTEX_CREATE_EXT: Include code for OSMutexCreateExt()"
    #endif
#endif
...
#error  "OS_CFG.H, Missing OS_CFG_Q_EN: Enable (1) or Disable (0) code generation for QUEUES"
    ...
    #ifndef OS_CFG_Q_CREATE_EXT
    #error  "OS_CFG.H, Missing OS_CFG_Q_CREATE_EXT: Include code for OSQCreateExt()"
    #endif
#endif
...
#error  "OS_CFG.H, Missing OS_CFG_SEM_EN: Enable (1) or Disable (0) code generation for SEMAPHORES"
    ...
    #ifndef OS_CFG_SEM_CREATE_EXT
    #error  "OS_CFG.H, Missing OS_CFG_SEM_CREATE_EXT: Include code for OSSemCreateExt()"
    #endif
#endif
...
#ifndef OS_CFG_TASK_STK_REDZONE_EN
#error  "OS_CFG.H, Missing OS_CFG_TASK_STK_REDZONE_EN: Include support for task stack redzone checking"
#endif

#ifndef OS_CFG_TASK_STK_REDZONE_DEPTH
#error  "OS_CFG.H, Missing OS_CFG_TASK_STK_REDZONE_DEPTH: Configure size of the redzone region"
#endif

#ifndef OS_CFG_TASK_STK_EXT_INFO
#error  "OS_CFG.H, Missing OS_CFG_TASK_STK_EXT_INFO: Enables the StkBasePtr and StkSize TCB members"
#endif
...
*/
                                                                ...
                                                                /* See 'os.h  Note #1a'.                              */
#if CPU_CORE_VERSION < 13200u
#error  "cpu_def.h, CPU_CORE_VERSION SHOULD be >= V1.32.00"
#endif
