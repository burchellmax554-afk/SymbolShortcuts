...
#if (OS_CREATE_EXT > 0u)
    p_grp->CreateOpt = OS_OPT_CREATE_PRIO;
#endif
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Don't allow blocking-pends from Timer Callbacks      */
        if ((opt & OS_OPT_PEND_NON_BLOCKING) != OS_OPT_PEND_NON_BLOCKING) {
           *p_err = OS_ERR_PEND_TMR;
            OS_TRACE_FLAG_PEND_FAILED(p_grp);
            OS_TRACE_FLAG_PEND_EXIT(OS_ERR_PEND_TMR);
            return ((OS_FLAGS)0);
        }
    }
#endif
...
*
* Description: This function is internal to Cs/OS3 and is used to put a task to sleep until the desired
...
*
* Description: These functions are called by Cs/OS3 to add or remove an event flag group from the event flag debug
*              list.
*
...
*
* Description: This function is internal to Cs/OS3 and is used to make a task ready-to-run because the desired event
