...
*                                                    CREATE A TASK
*
* Description: This function is used to have Cs/OS3 manage the execution of a task.  Tasks can either be created
...
*                             OS_ERR_TASK_DEL_IDLE           If you attempted to delete Cs/OS3's idle task
*                             OS_ERR_TASK_DEL_INVALID        If you attempted to delete Cs/OS3's ISR handler task
...
*
*              2) A self-deleting task may not execute lines after CPU_CRITICAL_EXIT() if an ISR invokes OSSched().
*                 If TLS is enabled, the call to OS_TLS_ReleaseLocks() is not required due to OS_MutexGrpPostAll().
*                 The call to OSSched() will be made by the interrupt which caused the task switch.
************************************************************************************************************************
*/
                 ...
                 OS_ERR  *p_err)
{
...
#if defined(OS_CFG_TLS_TBL_SIZE) && (OS_CFG_TLS_TBL_SIZE > 0u)
    OS_TLS_GlobalAcquire();                                     /* Stop other tasks from accessing TLS shared resources.*/
#endif
    ...
    switch (p_tcb->TaskState) {
        ...
        default:
            CPU_CRITICAL_EXIT();
#if defined(OS_CFG_TLS_TBL_SIZE) && (OS_CFG_TLS_TBL_SIZE > 0u)
            OS_TLS_GlobalRelease();                             /* Release TLS locks for other tasks.                   */
#endif
           *p_err = OS_ERR_STATE_INVALID;
            return;
    }
...
*                                OS_ERR_PEND_TMR           If you called this function from a Timer Callback
                    ...
                    OS_ERR       *p_err)
{
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Not allowed to call from Timer Callbacks             */
        OS_TRACE_TASK_MSG_Q_PEND_EXIT(OS_ERR_PEND_TMR);
                           ...
                           OS_ERR   *p_err)
{
    ...
    if (OSTCBCurPtr == &OSTmrTaskTCB) {                         /* Not allowed to call from Timer Callbacks             */
        OS_TRACE_TASK_SEM_PEND_FAILED(OSTCBCurPtr);
        OS_TRACE_TASK_SEM_PEND_EXIT(OS_ERR_PEND_TMR);
       *p_err = OS_ERR_PEND_TMR;
        return (0u);
    }
#endif
...
*
* Description: These functions are called by Cs/OS3 to add or remove an OS_TCB from the debug list.
*
...
* Returns    : If the stack is corrupted (OS_FALSE) or not (OS_TRUE).
*
* Note(s)    : 1) This function is INTERNAL to Cs/OS3 and your application should not call it.
************************************************************************************************************************
*/
